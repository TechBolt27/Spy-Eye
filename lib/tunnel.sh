...
cat <<EOF
    ${blue}<<------ ${white}Select a Tunnel Option ${blue}------>>${nocolor}

    ${cyan}[${blue}1${cyan}]${white} Cloudflared
    ${cyan}[${blue}2${cyan}]${white} Localhost
    ${cyan}[${blue}3${cyan}]${white} Back to Main Menu
...
_start_php_server() {
    ...
    if [[ -z "$PHP_BIN" ]]; then
        echo -e "    ${red}[${blue}!${red}]${white} PHP not found! Install with: sudo apt install php${nocolor}"
        exit 1
    fi
    ...
    echo -e "    ${cyan}[${blue}*${cyan}]${white} Starting PHP server (${green}$PHP_BIN${white})..."
    ...
    echo -e "    ${cyan}[${blue}+${cyan}]${white} Local server: ${green}http://127.0.0.1:9090${nocolor}"
}
...
start_cloudf() {
    { clear; banner | lolcat; banner2; echo; }
    echo -e "${blue}    <<------ ${cyan}Starting Cloudflared Tunnel ${blue}------>>${nocolor}"
    echo -e ""

    _start_php_server

    echo -e "    ${cyan}[${blue}*${cyan}]${white} Starting Cloudflared..."
    ...
    echo -ne "    ${cyan}[${blue}*${cyan}]${white} Generating link, please wait..."
    local MAX_WAIT=30
    local count=0
    local cf_link=""
    ...
    while true; do
        ...
        if [[ -n "$cf_link" ]]; then
            echo -e "\r    ${cyan}[${blue}+${cyan}]${white} Direct Link: ${green}$cf_link${nocolor}           "
            break
        fi
        count=$((count + 1))
...
start_localhst() {
    { clear; banner | lolcat; banner2; echo; }
    echo -e "${blue}    <<------ ${cyan}Starting Localhost Server ${blue}------>>${nocolor}"
    echo -e ""
